...
void update(float t)
{
	...
	if (returnMTV(*boxInfocus, *boxOutfocus, n))					// Check if the collision occours
	{
		if (boxOutfocus->inverseMass != 0.0f)
			...
			boxInfocus->origin -= n;								// If one of the object is immovable, then move the other object
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		box1.origin = glm::vec3(0.0f, 0.5f, 0.0f);
		box1.linearVelocity = glm::vec3(2.0f, 0.0f, 0.0f);
	}
}
#pragma endregion
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "3D linear friction", nullptr, nullptr);

	std::cout << "\n This program demonstrates implementation of linear friction \n\n\n\n\n\n\n\n\n\n";
